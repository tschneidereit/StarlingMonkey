...
#define WIZER_INIT(init_func)                                                  \
  ...
  /* This function replaces `_initialize` in a reactor module            */   \
  /* in the pre-initialized Wasm module.                                     */   \
  __attribute__((export_name("wizer.resume"))) void __wizer_resume() {         \
    /* In reactor mode, _initialize should not call main(). The          */     \
    /* pre-initialized state is restored by wizer, and then exports     */     \
    /* (like incoming-handler or cli/run) are called directly.          */     \
    /* We only need to do nothing here — wizer restores the memory.     */     \
  }
