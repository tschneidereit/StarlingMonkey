...
class WasmtimeInstance {
  private static _wasmtime: ChildProcessWithoutNullStreams;
  ...
  static async start(workspaceFolder: string, component: string, config: IComponentRuntimeConfig) {
    assert(!this.running, "Wasmtime is already running");
    this.running = true;
    ...
    // Start wasmtime as a new process
    ...
    args.push(component);
    // console.debug(`(cd ${path} && wasmtime ${args.join(" ")})`);
    this._wasmtime = spawn(config.executable, args);

    this._wasmtime.stdout.on("data", (data) => {
      console.log(`wasmtime ${data}`);
    });

    this._wasmtime.stderr.on("data", (data) => {
      console.error(`wasmtime ${data}`);
    });

    this._wasmtime.on("close", (code) => {
...
export class StarlingMonkeyRuntime extends EventEmitter {
  ...
  public async start(
program: string, component: string, stopOnEntry: boolean, debug: boolean  ): Promise<void> {
    await this.startWasmtime(component);
  ...
  startSessionServer(): void {
    ...
    this._server = Net.createServer((socket) => {
      ...
      socket.on("data", async (data) => {
        if (!debuggerScriptSent) {
          ...
          let debuggerScript = await this._fileAccessor.readFile(`${extensionDir}/starling-debugger/debugger.js`);
          socket.write(`${debuggerScript.length}\n${debuggerScript}`);
        ...
        try {
          ...
          resetMessageState();
          this._messageReceived.resolve(message);
        } catch (e) {
    ...
    let port = (<Net.AddressInfo>this._server.address()).port;
    WasmtimeInstance.setNextSessionPort(port);
  }

  private async startWasmtime(component: string) {
    if (WasmtimeInstance.running) {
      assert(
        WasmtimeInstance.workspaceFolder === this._workspaceDir,
        "Wasmtime is already running in a different workspace"
      );
      return;
    }
    await WasmtimeInstance.start(this._workspaceDir, component, this._config.componentRuntime);
  }
