...
* Ensure that a build of StarlingMonkey is available with the path `debugger/vscode-dap-extension/out/main.wasm`. This is configurable via the `component` property of the launch configuration in `launch.json`[^2]
  (Alternatively, you can update the `component` property of the launch configuration in `launch.json` to point to a different file.)
...
[^1]: Alternatively, you can use another WebAssembly Components runtime, as long as it supports outgoing TCP socket connections, and passing environment variables to the guest. In that case, you'll have to update the [configuration](#configuration)
