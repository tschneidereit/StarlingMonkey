...
use base::generic_channel::{GenericSender, OpaqueSender, RoutedReceiver};
...
pub(crate) enum ScriptEventLoopSender {
    /// A sender that sends to the StarlingMonkey event loop.
    Starling(Sender<WorkerScriptMsg>),
...
impl ScriptEventLoopSender {
    ...
    pub(crate) fn send(&self, message: CommonScriptMsg) -> Result<(), SendError<()>> {
        match self {
            Self::Starling(sender) => sender
                .send(WorkerScriptMsg::Common(message))
                .map_err(|_| SendError(())),
...
pub(crate) enum ScriptEventLoopReceiver {
    /// A receiver that receives messages to the StarlingMonkey event loop.
    Starling(Receiver<WorkerScriptMsg>),
...
impl QueuedTaskConversion for WorkerScriptMsg {
    ...
    fn inactive_msg() -> Self {
        ...
        panic!("StarlingMonkey should never receive messages marked as inactive");
    }
    ...
    fn is_wake_up(&self) -> bool {
        matches!(self, WorkerScriptMsg::WakeUp)
    }
}
