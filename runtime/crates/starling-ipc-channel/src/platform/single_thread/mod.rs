...
// Callback type for receivers
type ReceiverCallback = Box<dyn FnMut(IpcMessage) + Send>;

// Global registry for receiver callbacks
// Maps receiver ID -> callback
static RECEIVER_CALLBACKS: LazyLock<Mutex<HashMap<u64, ReceiverCallback>>> =
    LazyLock::new(|| Mutex::new(HashMap::new()));

// Global counter for receiver IDs
static NEXT_RECEIVER_ID: LazyLock<Mutex<u64>> = LazyLock::new(|| Mutex::new(0));

fn allocate_receiver_id() -> u64 {
    let mut id = NEXT_RECEIVER_ID.lock().unwrap();
    let current = *id;
    *id += 1;
    current
}
...
pub fn channel() -> Result<(OsIpcSender, OsIpcReceiver), ChannelError> {
    ...
    let receiver_id = allocate_receiver_id();
    Ok((
        OsIpcSender::new(base_sender, receiver_id),
        OsIpcReceiver::new(base_receiver, receiver_id),
    ))
}
...
impl OsIpcReceiver {
    fn new(receiver: Receiver<ChannelMessage>, id: u64) -> OsIpcReceiver {
        OsIpcReceiver {
    ...
    pub fn register_callback(&self, mut callback: ReceiverCallback) {
        // First, drain any existing messages from the channel and process them with the callback
        let r = self.receiver.borrow();
        if let Some(ref receiver) = *r {
            while let Ok(ChannelMessage(ipc_message)) = receiver.try_recv() {
                callback(ipc_message);
            }
        }
        drop(r);
...
impl OsIpcSender {
    fn new(sender: Sender<ChannelMessage>, receiver_id: u64) -> OsIpcSender {
        OsIpcSender { sender, receiver_id }
    }
    ...
    pub fn send(
        ...
        if let Some(callback) = callbacks.get_mut(&self.receiver_id) {
            ...
            // No callback, send to channel as usual
            drop(callbacks); // Release lock before sending
            self.sender
