...
impl HttpState {
    pub(crate) fn memory_reports(&self, _suffix: &str, _ops: &mut MallocSizeOfOps) -> Vec<Report> {
        vec![
            // Report {
...
) -> Result<(HyperResponse<Decoder>, Option<ChromeToDevtoolsControlMsg>), NetworkError> {
    {
        ...
        *request.headers_mut() = headers.clone();
        ...
        client
            ...
            .map_err(move |_error| {
                NetworkError::Internal("TODO: Figure out error handling here".into())
...
async fn http_network_or_cache_fetch(
    ...
    if response.is_none() {
        ...
        if response.is_none() {
            ...
            let _forward_response = response.insert(forward_response);
