...
//! 
//! Except, for WASI targets, where we fall back to using `std::time::Instant` because
//! WASI does not have the concept of multiple processes and thus no need for cross-process timers.
...
#[cfg(all(any(unix, target_family = "wasm"), not(any(target_os = "macos", target_os = "ios"))))]
mod platform {
    use libc::timespec;
